...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if area not in VALID_AREAS or not isinstance(items, list) or len(items) == 0:
        return func.HttpResponse(
            json.dumps({"error": "Invalid order payload"}),
            status_code=400,
            mimetype="application/json"
        )
    ...
    for it in items:
        ...
        if not meal_id or not isinstance(qty, int) or qty <= 0:
            return func.HttpResponse(
                json.dumps({"error": "Invalid items"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        try:
            ...
            return func.HttpResponse(
                json.dumps({"error": f"Meal not found: {meal_id}"}),
                status_code=400,
                mimetype="application/json"
            )
    ...
    # Simple ETA calculation (per your original brief)
    eta = sum(prep_times) + 5 + 15
